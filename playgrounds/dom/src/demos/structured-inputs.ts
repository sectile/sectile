...
function mountTagsInput(context: DemoContext, initial: readonly string[], maxTags: number, controlled: boolean): DemoSession {
  ...
  let value = [...initial]; let inputValue = ''; let renderedTags: HTMLButtonElement[] = []; let connection!: TagsInputConnection;
  ...
  function render(): void { const { revision, state } = connection.getSnapshot(); renderedTags.forEach((tag) => tag.remove()); renderedTags = state.tags.map((tag, index) => { const button = document.createElement('button'); button.type = 'button'; button.className = 'tag-token'; button.disabled = context.interaction.disabled === true; button.append(document.createTextNode(tag), createElement(X, { 'aria-hidden': 'true', height: 13, width: 13 })); connection.setTagAttributes(button, index); button.classList.toggle('current', state.current === index); root.insertBefore(button, input); return button; }); hint.textContent = `${state.tags.length} of ${maxTags} tags · Enter or comma adds the draft`; context.showState(revision, { tags: state.tags, draft: state.draft, current: state.current, maxTags, ownership: controlled ? 'controlled' : 'uncontrolled' }); }
