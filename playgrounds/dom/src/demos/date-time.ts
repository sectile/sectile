...
function mountRangePicker(context: DemoContext, scenario: { initial: DateRange | null; min?: DateValue; max?: DateValue; controlled?: boolean }): DemoSession {
  const ui = pickerShell(true); context.surface.append(ui.shell); let controlledValue = scenario.initial; let controlledHighlight = scenario.initial?.end ?? date(2026, 8, 22); let controlledOpen = true; let syncScheduled = false; let connection: DateRangePickerConnection;
  connection = createDateRangePicker({ root: ui.popup, grid: ui.grid, trigger: ui.trigger, startInput: ui.startInput, ...(ui.endInput === undefined ? {} : { endInput: ui.endInput }), ...context.interaction, policies: { ...(scenario.min === undefined ? {} : { min: scenario.min }), ...(scenario.max === undefined ? {} : { max: scenario.max }) }, ...(scenario.controlled ? { value: controlledValue, highlightedValue: controlledHighlight, open: controlledOpen } : { defaultValue: controlledValue, defaultHighlightedValue: controlledHighlight, defaultOpen: true }), onValueChange: (value) => { controlledValue = value; scheduleSync(); }, onHighlightedValueChange: (value) => { controlledHighlight = value; scheduleSync(); }, onOpenChange: (open) => { controlledOpen = open; scheduleSync(); }, onUpdate: render });
  ui.previous.addEventListener('click', previous); ui.next.addEventListener('click', next); render();
  function previous(): void { connection.handleEvent('previous-month'); } function next(): void { connection.handleEvent('next-month'); }
  function scheduleSync(): void { if (!scenario.controlled || syncScheduled) return; syncScheduled = true; queueMicrotask(() => { syncScheduled = false; sync(); }); }
...
function mountDateTimeRangePicker(context: DemoContext, scenario: { initial: DateTimeRange; controlled?: boolean }): DemoSession {
  ...
  connection = createDateTimeRangePicker({ root: ui.popup, grid: ui.grid, trigger: ui.trigger, startDateInput: ui.startInput, ...(ui.endInput === undefined ? {} : { endDateInput: ui.endInput }), startTimeInput: ui.startTimeInput, ...(ui.endTimeInput === undefined ? {} : { endTimeInput: ui.endTimeInput }), ...context.interaction, ...(scenario.controlled ? { value: controlledValue, highlightedValue: controlledHighlight, open: controlledOpen } : { defaultValue: controlledValue, defaultHighlightedValue: controlledHighlight, defaultOpen: true }), onValueChange: (value) => { controlledValue = value; scheduleSync(); }, onHighlightedValueChange: (value) => { controlledHighlight = value; scheduleSync(); }, onOpenChange: (open) => { controlledOpen = open; scheduleSync(); }, onUpdate: render });
  ui.previous.addEventListener('click', previous); ui.next.addEventListener('click', next); render();
  function previous(): void { connection.handleEvent('previous-month'); } function next(): void { connection.handleEvent('next-month'); }
  function scheduleSync(): void { if (!scenario.controlled || syncScheduled) return; syncScheduled = true; queueMicrotask(() => { syncScheduled = false; sync(); }); }
  function sync(): void { connection.syncControlledValues({ value: controlledValue, highlightedValue: controlledHighlight, open: controlledOpen }); }
