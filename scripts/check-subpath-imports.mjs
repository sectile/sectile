...
try {
  ...
  await symlink(resolve('packages/primitives'), join(scope, 'primitives'), 'dir');
  await writeFile(join(directory, 'consumer.mjs'), `
    ...
    for (const value of [createSequence, createRange, createGrid, createTree]) {
  ...
  console.log(JSON.stringify({ status: 'passed', subpaths: 5, typeConsumer: 'passed' }, null, 2));
} finally {
