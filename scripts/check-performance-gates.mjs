...
assert.equal(manifest.schemaVersion, 2, 'unsupported performance gate schema');
assert.equal(manifest.certificationCommand, 'pnpm performance:certify');
assert.equal(manifest.targetedCommand, 'pnpm performance:check -- <package>');
...
assert.deepEqual(manifest.certificationWhen, [
  ...
  'an explicit full-performance certification request',
]);
