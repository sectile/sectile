...
for (const [templateID, contract] of Object.entries(templateDocument.templates)) {
  await validateEvidenceReferences(contract.evidence, repoRoot, `template:${templateID}`);
}
const fragments = [];
...
for (const packageName of PACKAGE_NAMES) {
  ...
  for (const fragment of fragments.filter((entry) => entry.package === packageName && entry.kind === 'internal-operations')) {
    for (const operation of fragment.operations) {
      ...
      const expanded = expandOperation(operation, templateDocument.templates);
      await validateEvidenceReferences(expanded.contract.evidence, repoRoot, `${packageName}:${operation.id}`);
