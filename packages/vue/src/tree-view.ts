...
export interface TreeViewRootSlotProps { readonly value: readonly string[]; readonly expandedValue: readonly string[]; readonly highlightedValue: string | null; readonly disabled: boolean; readonly: boolean }
...
interface Context {
  ...
  registerItem(element: HTMLElement, id: string, level: number | undefined, disabled: boolean): void;
...
export const TreeViewRoot = defineComponent({
  ...
  emits: { 'update:modelValue': (_value: readonly string[]): boolean => true, 'update:expandedValue': (_value: readonly string[]): boolean => true, 'update:highlightedValue': (_value: string | null): boolean => true },
  ...
  setup(props, { attrs, emit, slots }) {
    ...
    const controlled = { value: props.modelValue !== undefined, expanded: props.expandedValue !== undefined, highlighted: props.highlightedValue !== undefined };
    ...
    const refreshParts = (): void => {
      ...
      element.value.querySelectorAll<HTMLElement>('[data-sectile-tree-view-item]').forEach((node) => { const id = node.dataset['sectileTreeViewItem']; const level = numberData(node.dataset['sectileTreeViewLevel']); if (id !== undefined) connection.value?.setItemAttributes(node, { id, ...(level === undefined ? {} : { level }), disabled: node.dataset['disabled'] !== undefined }); });
    ...
    const connect = (): void => {
      ...
      connection.value = createTreeView({
        root: element.value, nodes: props.nodes,
        ...(controlled.value ? { value: props.modelValue as readonly string[] } : { defaultValue: localValue.value }),
        ...(controlled.expanded ? { expandedValue: props.expandedValue as readonly string[] } : { defaultExpandedValue: localExpanded.value }),
        ...
        onExpandedValueChange: ({ value }) => { localExpanded.value = value; emit('update:expandedValue', value); },
    ...
    provide<Context>(key, {
      state, disabledItems,
      registerItem: (node, id, level, disabled) => connection.value?.setItemAttributes(node, { id, ...(level === undefined ? {} : { level }), disabled }),
    ...
    watch([() => props.nodes, () => props.disabledItems, () => props.disabled, () => props.readonly, () => props.label, () => props.policies], connect);
    watch([() => props.modelValue, () => props.expandedValue, () => props.highlightedValue], () => {
      if (connection.value === undefined) return;
      const result = connection.value.syncControlledValues({ ...(controlled.value ? { value: props.modelValue } : {}), ...(controlled.expanded ? { expandedValue: props.expandedValue } : {}), ...(controlled.highlighted ? { highlightedValue: props.highlightedValue } : {}) });
...
export const TreeViewGroup = defineComponent({ name: 'SectileTreeViewGroup', inheritAttrs: false, props: partProps, slots: Object as SlotsType<{ default: (props: TreeViewRootSlotProps) => VNodeChild }>, setup(props, { attrs, slots }) { const root = useRoot('TreeViewGroup'); return (): VNodeChild => h(Primitive, mergeProps(attrs, { as: props.as, asChild: props.asChild, role: 'group', 'data-scope': 'tree-view', 'data-part': 'group' }), { default: () => slots['default']?.(root.state.value) }); } });

export const TreeViewItem = defineComponent({
  name: 'SectileTreeViewItem', inheritAttrs: false,
  props: { value: { type: String, required: true }, level: { type: Number, default: undefined }, disabled: { type: Boolean, default: false }, ...partProps },
  slots: Object as SlotsType<{ default: (props: TreeViewItemSlotProps) => VNodeChild }>,
  setup(props, { attrs, slots }) { const root = useRoot('TreeViewItem'); const state = computed<TreeViewItemSlotProps>(() => ({ expandedValue: root.state.value.expandedValue, highlightedValue: root.state.value.highlightedValue, readonly: root.state.value.readonly, value: props.value, selectedValues: root.state.value.value, selected: root.state.value.value.includes(props.value), expanded: root.state.value.expandedValue.includes(props.value), highlighted: root.state.value.highlightedValue === props.value, disabled: root.state.value.disabled || props.disabled || root.disabledItems.value.has(props.value) })); return (): VNodeChild => h(Primitive, mergeProps(attrs, {
    as: props.as, asChild: props.asChild, elementRef: (node: unknown) => { if (node instanceof HTMLElement) root.registerItem(node, props.value, props.level, props.disabled); },
    'data-sectile-tree-view-item': props.value, 'data-sectile-tree-view-level': props.level, 'data-scope': 'tree-view', 'data-part': 'item',
...
export const TreeViewDisclosure = defineComponent({
  ...
  slots: Object as SlotsType<{ default: (props: TreeViewItemSlotProps) => VNodeChild }>,
  setup(props, { attrs, slots }) { const root = useRoot('TreeViewDisclosure'); const state = computed<TreeViewItemSlotProps>(() => ({ expandedValue: root.state.value.expandedValue, highlightedValue: root.state.value.highlightedValue, readonly: root.state.value.readonly, value: props.for, selectedValues: root.state.value.value, selected: root.state.value.value.includes(props.for), expanded: root.state.value.expandedValue.includes(props.for), highlighted: root.state.value.highlightedValue === props.for, disabled: root.state.value.disabled || root.disabledItems.value.has(props.for) })); return (): VNodeChild => h(Primitive, mergeProps(attrs, {
...
function numberData(value: string | undefined): number | undefined { if (value === undefined) return undefined; const parsed = Number(value); return Number.isFinite(parsed) ? parsed : undefined; }
...
export type { TreeNodeInput, TreeViewPolicies };
