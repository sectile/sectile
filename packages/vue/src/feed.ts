...
export const FeedRoot = defineComponent({
  ...
  setup(props, { attrs, emit, slots }) {
    ...
    const highlighted = shallowRef<string | null>(props.defaultHighlightedValue ?? props.items[0] ?? null); const pending = shallowRef<FeedDirection | null>(null); const currentRevision = shallowRef(props.revision);
    const state = computed<FeedRootSlotProps>(() => Object.freeze({ highlightedValue: highlighted.value, pending: pending.value, revision: currentRevision.value, disabled: props.disabled }));
    ...
    const refresh = (): void => { const snapshot = connection.value?.getSnapshot().state; if (snapshot === undefined) return; highlighted.value = snapshot.cursor.current; pending.value = snapshot.pending; currentRevision.value = snapshot.revision; refreshParts(); };
    const connect = (): void => {
      ...
      connection.value = createFeed({
        ...
        onRequestWindow: (direction, anchor, revision) => { pending.value = direction; emit('request-window', direction, anchor, revision); }, onUpdate: refresh,
      });
    ...
    watch([() => props.items, () => props.revision], () => {
      if (connection.value === undefined) return;
      if (props.revision <= currentRevision.value) { connect(); return; }
      const result = connection.value.syncWindow({ items: props.items, revision: props.revision, highlightedValue: highlighted.value });
