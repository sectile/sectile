...
function response(request) { const start = request.access.kind === 'page' ? (request.access.page - 1) * request.access.itemsPerPage : request.access.start; const count = request.access.kind === 'page' ? request.access.itemsPerPage : request.access.count; return { protocolVersion: 1, requestID: request.requestID, sourceGeneration: request.sourceGeneration, queryRevision: request.queryRevision, expansionRevision: request.expansionRevision, viewRevision: 1, access: request.access, matchingLeafCount: { kind: 'known', value: rows.length }, visibleRowCount: { kind: 'known', value: rows.length }, rows: rows.slice(start, start + count), columnSchema: { revision: request.columnSchemaRevision, columns, headers: [] }, removedRowIDs: [] }; }
