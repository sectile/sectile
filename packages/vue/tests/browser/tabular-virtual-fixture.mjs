...
function readyTable() { const controller = useDataTable({ columns }); const request = controller.requestState.value.pendingRequest; if (request === null) throw new Error('missing request'); const result = controller.synchronizeView(response(request)); if (!result.ok) throw new Error(result.error.message); return controller; }
function readyGrid() { const controller = useDataGrid({ columns }); const request = controller.requestState.value.pendingRequest; if (request === null) throw new Error('missing request'); const result = controller.synchronizeView(response(request)); if (!result.ok) throw new Error(result.error.message); return controller; }
