...
test('Vue Tabular source cancels stale work and exposes resolver errors without rendering policy', async () => {
  ...
  const app = createApp({ setup() { controller = useDataTable({ columns }); const DataTable = createDataTableComponents(controller); source = useDataTableSource(controller, (request, { signal }) => new Promise((resolve) => { release = () => resolve(response(request, signal.aborted ? 'stale' : 'late')); })); return () => h(DataTable.Provider, null, { default: () => h(DataTable.Root) }); } });
