...
class DOMCheckedControlImpl<State, Event, Command, Value> implements DOMCheckedControl<State, Event, Value> {
  ...
  public handleEvent(event: Event): boolean { this.#runtime.handle(event); this.updateAttributes(); this.#options.onUpdate?.(); return true; }
  public updateAttributes(): void { this.#options.element.setAttribute(this.#options.attribute, this.#options.format(this.#options.read(this.#runtime.getSnapshot().state))); }
