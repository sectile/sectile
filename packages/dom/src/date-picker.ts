...
import { applyDatePickerEvent, createDatePickerMonth, createDatePickerState, createDatePickerWeek, createDatePickerYear, datePickerID, isDatePickerValueAvailable, type DatePickerCommand, type DatePickerEvent, type DatePickerMonthValue, type DatePickerPolicies, type DatePickerState } from '@sectile/core/date-picker';
...
class DOMDatePicker implements DatePickerConnection {
  ...
  public getWeek(): readonly DateValue[] { return unwrap(createDatePickerWeek(this.getSnapshot().state.highlighted, this.options.policies?.weekStartsOn)); }
  public getYear(): readonly (readonly DatePickerMonthValue[])[] { return unwrap(createDatePickerYear(this.getSnapshot().state.view.year)); }
