...
function tryCreateSelectConnection<ID extends StableID>(options: SelectOptions<ID>): Result<SelectConnection<ID>> {
  ...
  const runtime = createSemanticController<SelectState<ID>, SelectEvent<ID>, SelectCommand<ID>, SelectEffect<ID>>({
    ...
    initial: tryCreateSelectState(domain.value, { value: options.value ?? options.defaultValue ?? null, current: options.highlightedValue ?? options.defaultHighlightedValue ?? options.value ?? options.defaultValue ?? null, open: options.open ?? options.defaultOpen ?? false }),
...
class DOMSelectConnection<ID extends StableID> implements SelectConnection<ID> {
  ...
  handleEvent(event: SelectEvent<ID>): boolean { const result = this.#runtime.handle(event); if (!result.ok) return false; this.#render(); for (const effect of result.commands) { if (effect.type === 'focus-option') queueMicrotask(() => { if (!this.#active) return; for (const element of this.#options.popup.querySelectorAll<HTMLElement>('[data-select-id]')) if (element.dataset['selectId'] === stableIDToken(effect.id)) element.focus(); }); else this.#options.trigger.focus(); } this.#options.onUpdate?.(); return true; }
