...
class DOMSelectConnection<ID extends StableID> implements SelectConnection<ID> {
  ...
  disconnect(): void { this.#layer.disconnect(); this.#options.trigger.removeEventListener('click', this.#triggerClick); this.#options.root.removeEventListener('keydown', this.#keydown); this.#options.popup.removeEventListener('click', this.#click); }
  #render(): void { const open = this.#runtime.getSnapshot().state.open; this.#options.trigger.setAttribute('aria-expanded', String(open)); this.#options.popup.hidden = !open; this.#layer.sync(); }
}
