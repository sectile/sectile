...
export interface TimeFieldOptions { readonly input: HTMLInputElement; readonly policies?: TimeFieldPolicies; readonly value?: TimeValue | null; readonly defaultValue?: TimeValue | null; readonly inputState?: TextEditingState; readonly defaultInputState?: TextEditingState; readonly disabled?: boolean; readonly readOnly?: boolean; readonly required?: boolean; readonly label?: string; readonly native?: boolean; readonly onValueChange?: (value: TimeValue | null) => boolean | void; readonly onInputStateChange?: (value: TextEditingState, previousValue: TextEditingState) => void; readonly onUpdate?: () => void; }
...
export interface TimeFieldConnection { getSnapshot(): RevisionSnapshot<TimeFieldState>; getText(): string; getValue(): TimeValue | null; syncControlledValues(values: TimeFieldControlledValues): DOMTemporalResult<RevisionSnapshot<TimeFieldState>>; handleEvent(event: TimeFieldEvent): boolean; refresh(): void; disconnect(): void; }
...
export function tryCreateTimeField(options: TimeFieldOptions): DOMTemporalResult<FacadeConnection<TimeFieldConnection>> { return createDOMTemporalFacadeConnection(options, construct); }
function construct(options: TimeFieldOptions): DOMTemporalResult<TimeFieldConnection> {
...
class DOMTimeField implements TimeFieldConnection {
  readonly options: TimeFieldOptions; readonly runtime: DOMTemporalController<TimeFieldState, TimeFieldEvent, TimeFieldCommand>; readonly valueControlled: boolean; readonly inputControlled: boolean;
  ...
  public syncControlledValues(values: TimeFieldControlledValues): DOMTemporalResult<RevisionSnapshot<TimeFieldState>> { if (this.valueControlled !== (values.value !== undefined) || this.inputControlled !== (values.inputState !== undefined)) return { ok: false, error: { class: 'construction', code: 'controlled-shape-mismatch', message: 'Controlled time field values must preserve their construction-time shape.' } }; const state = this.getSnapshot().state; const value = this.valueControlled ? values.value as TimeValue | null : state.value; const inputState = this.inputControlled ? values.inputState as TextEditingState : synchronizeControlledFieldInput(synchronizeFieldInputSelection(state.inputState, this.options.input), state.value === null ? '' : formatTimeValue(state.value), value === null ? '' : formatTimeValue(value)); const result = this.runtime.replace(tryCreateTimeFieldState(value, inputState)); if (result.ok) { this.refresh(); this.options.onUpdate?.(); } return result; }
  public handleEvent(event: TimeFieldEvent): boolean { const previous = this.getSnapshot(); const result = this.runtime.handle(event); setFieldValidity(this.options.input, result); if (result.ok) { for (const command of result.commands) if (command.type === 'value-committed' && this.options.onValueChange?.(command.value) === false) { this.runtime.replace({ ok: true, value: previous.state }); this.refresh(); setFieldValidity(this.options.input, { ok: false, error: { message: 'The parent control rejected the time value.' } }); this.options.onUpdate?.(); return false; } this.refresh(); this.options.onUpdate?.(); } return result.ok; }
