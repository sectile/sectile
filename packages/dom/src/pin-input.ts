...
function tryCreatePinInputConnection(options: PinInputOptions): Result<PinInputConnection> {
  ...
  const runtime = createSemanticController<PinInputState, PinInputEvent, PinInputCommand, PinInputEffect>({
    ...
    reconcile: (previous, proposed) => controlled
      ? pinInputStateAtCurrent(options.inputs.length, previous.values, proposed.current)
...
function pinInputStateAtCurrent(length: number, value: string | readonly string[], current: number): Result<PinInputState> {
  const state = tryCreatePinInputState(length, value);
  return state.ok
    ? { ok: true, value: Object.freeze({ values: state.value.values, current }) }
    : state;
}
