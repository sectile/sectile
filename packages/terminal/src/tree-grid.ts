...
>(
  ...
  return runtime.ok ? { ok: true, value: new TerminalTreeGridController(options, runtime.value) } : runtime;
}
...
class TerminalTreeGridController<RowID extends StableID, CellID extends StableID>
  ...
  public handleKeyboardInput(
    ...
    if (event === null) {
      return this.#runtime.reject('unsupported-terminal-key', 'Terminal keyboard input does not map to a tree-grid semantic event.', { key: input.key });
    }
