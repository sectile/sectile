...
function construct(options: DatePickerOptions): TerminalTemporalResult<DatePickerConnection> {
  ...
  const runtime = createTerminalTemporalController<DatePickerState, DatePickerEvent, DatePickerCommand, DatePickerCommand>({ initial, reducer: (state, event) => applyGranularPickerEvent(state, event, policies, granularity), reconcile: (previous, proposed) => tryCreateGranularPickerState({ value: controls.value ? previous.value : proposed.value, highlighted: controls.highlighted ? previous.highlighted : proposed.highlighted, view: controls.highlighted ? previous.view : proposed.view, viewMode: proposed.viewMode, open: controls.open ? previous.open : proposed.open }, granularity), notify: [
