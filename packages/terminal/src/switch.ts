...
import type { Result } from '@sectile/core'; import { applySwitchEvent, createSwitchState, type SwitchCommand, type SwitchEvent, type SwitchState } from '@sectile/core/switch'; import { createTerminalCheckedControl, type TerminalCheckedControl } from './internal/checked-control.js';
...
function tryCreateSwitchConnection(options: SwitchOptions = {}): Result<SwitchConnection> { return createTerminalCheckedControl<SwitchState, SwitchEvent, SwitchCommand, boolean>({ controlled: options.checked !== undefined, initial: createSwitchState(options.checked ?? options.defaultChecked ?? false), toggleEvent: 'toggle', reducer: applySwitchEvent, create: createSwitchState, read: (state) => state.checked, interaction: options, onChange: options.onCheckedChange, onUpdate: options.onUpdate }); }
