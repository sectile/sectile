...
function tryCreatePopoverConnection(options: PopoverOptions): Result<PopoverConnection> { return createTerminalPopup<PopoverState, PopoverEvent, PopoverCommand>({ controlled: options.open !== undefined, initial: createPopoverState(options.open ?? options.defaultOpen ?? false), reducer: applyPopoverEvent, create: createPopoverState, read: (state) => state.open, close: 'close', interaction: options, onOpenChange: options.onOpenChange, command: (command) => command.type === 'request-initial-focus' ? options.onInitialFocus?.() : options.onFocusRestore?.(), onUpdate: options.onUpdate }); }
