...
export interface AccordionOptions<ID extends StableID = StableID> {
  readonly items: readonly ID[]; readonly policies?: AccordionPolicies<ID>;
...
export function createAccordion<ID extends StableID>(options: AccordionOptions<ID>): Result<AccordionConnection<ID>> {
  ...
  const runtime = createSemanticController<AccordionState<ID>, AccordionEvent<ID>, AccordionCommand<ID>, AccordionCommand<ID>>({
    initial: createAccordionState(domain.value, { openIDs: initialOpen, current: options.highlightedValue !== undefined ? options.highlightedValue : options.defaultHighlightedValue ?? null }, options.policies),
    reducer: (state, event) => applyAccordionEvent(domain.value, state, event, options.policies),
    reconcile: (previous, proposed) => createAccordionState(domain.value, { openIDs: openControlled ? previous.openIDs : proposed.openIDs, current: highlightControlled ? previous.cursor.current : proposed.cursor.current }, options.policies),
  ...
  return { ok: true, value: new TerminalAccordionConnection(options, domain.value, runtime.value, openControlled, highlightControlled) };
}
...
class TerminalAccordionConnection<ID extends StableID> implements AccordionConnection<ID> {
  readonly #options: AccordionOptions<ID>; readonly #domain: Sequence<ID>; readonly #runtime: SemanticController<AccordionState<ID>, AccordionEvent<ID>, AccordionCommand<ID>>; readonly #openControlled: boolean; readonly #highlightControlled: boolean;
  public constructor(options: AccordionOptions<ID>, domain: Sequence<ID>, runtime: SemanticController<AccordionState<ID>, AccordionEvent<ID>, AccordionCommand<ID>>, openControlled: boolean, highlightControlled: boolean) { this.#options = options; this.#domain = domain; this.#runtime = runtime; this.#openControlled = openControlled; this.#highlightControlled = highlightControlled; }
  ...
  public syncControlledValues(values: { readonly openIDs?: readonly ID[]; readonly highlightedValue?: ID | null }): Result<RevisionSnapshot<AccordionState<ID>>> {
    ...
    const state = this.#runtime.getSnapshot().state; const result = this.#runtime.replace(createAccordionState(this.#domain, { openIDs: this.#openControlled ? (values.openIDs as readonly ID[]) : state.openIDs, current: this.#highlightControlled ? (values.highlightedValue as ID | null) : state.cursor.current }, this.#options.policies)); if (result.ok) this.#options.onUpdate?.(); return result;
  }
