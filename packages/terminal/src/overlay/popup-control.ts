...
export function createTerminalPopup<State, Event, Command extends object>(options: Options<State, Event, Command>): Result<TerminalPopupConnection<State, Event>> {
  const runtime = createControlledComponentController<State, Event, Command, boolean>({
    ...
    read: options.read,
    ...(options.command === undefined ? {} : { publishEffect: options.command }),
...
class TerminalPopup<State, Event, Command extends object> implements TerminalPopupConnection<State, Event> {
  ...
  public constructor(options: Options<State, Event, Command>, runtime: ControlledComponentController<State, Event, Command, boolean>) {
