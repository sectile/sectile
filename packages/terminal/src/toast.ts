...
function tryCreateToastConnection<ID extends StableID>(options: ToastOptions<ID>): Result<ToastConnection<ID>> {
  const runtime = createSemanticController<ToastState<ID>, ToastEvent<ID>, ToastCommand<ID>, ToastCommand<ID>>({ initial: tryCreateToastState(options.initialToasts ?? [], false, options), reducer: (state, event) => applyToastEvent(state, event, options), notify: (_previous, proposed) => options.onItemsChange?.(proposed.items), toEffect: (command) => command });
