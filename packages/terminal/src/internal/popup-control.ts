...
export function createTerminalPopup<State, Event, Command>(options: Options<State, Event, Command>): Result<TerminalPopupConnection<State, Event>> {
  const runtime = createSemanticController<State, Event, Command, Command>({ initial: options.initial, reducer: options.reducer, reconcile: (previous, proposed) => options.create(options.controlled ? options.read(previous) : options.read(proposed), proposed), notify: (previous, proposed) => { if (options.read(previous) !== options.read(proposed)) options.onOpenChange?.(options.read(proposed)); }, toEffect: (command) => command, interaction: options.interaction });
