...
export function createTerminalPopup<State, Event, Command>(options: Options<State, Event, Command>): Result<TerminalPopupConnection<State, Event>> {
  const runtime = createSemanticController<State, Event, Command, Command>({ initial: options.initial, reducer: options.reducer, reconcile: (previous, proposed) => options.create(options.controlled ? options.read(previous) : options.read(proposed)), notify: (previous, proposed) => { if (options.read(previous) !== options.read(proposed)) options.onOpenChange?.(options.read(proposed)); }, toEffect: (command) => command });
...
class TerminalPopup<State, Event, Command> implements TerminalPopupConnection<State, Event> {
  ...
  public handleEvent(event: Event): boolean { const result = this.#runtime.handle(event); if (result.ok) for (const command of result.commands) this.#options.command?.(command); this.#options.onUpdate?.(); return true; }
