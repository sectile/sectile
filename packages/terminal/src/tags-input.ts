...
class TerminalTagsInputConnection implements TagsInputConnection {
  ...
  readonly #controlled: { value: boolean; input: boolean };

  constructor(
    options: TagsInputOptions,
    runtime: SemanticController<TagsInputState, TagsInputEvent, TagsInputCommand>,
    controlled: { value: boolean; input: boolean },
  ) {
  ...
  syncControlledValues(values: { readonly value?: readonly string[]; readonly inputValue?: string }) {
    if (
      this.#controlled.value !== (values.value !== undefined)
      || this.#controlled.input !== (values.inputValue !== undefined)
    ) {
      return {
        ok: false as const,
        error: {
  ...
  handleEvent(event: TagsInputEvent) {
    const result = this.#runtime.handle(event);
    if (result.ok) this.#options.onUpdate?.();
    return result.ok;
  }

  handleKeyboardInput(input: TerminalKeyboardInput) {
    const state = this.#runtime.getSnapshot().state;
    const event: TagsInputEvent | null = input.key === 'enter' || input.text === ','
      ? { type: 'add' }
      : input.key === 'backspace'
        ? state.draft.length > 0
          ? { type: 'input', value: state.draft.slice(0, -1) }
          : state.current === null ? 'previous' : 'remove-current'
        : input.key === 'left'
          ? 'previous'
          : input.key === 'right'
            ? 'next'
            : input.text !== undefined
              ? { type: 'input', value: `${state.draft}${input.text}` }
              : null;
