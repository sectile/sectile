...
class TerminalDateRangePicker implements DateRangePickerConnection {
  readonly options: DateRangePickerOptions; readonly runtime: TerminalTemporalController<DateRangePickerState, DateRangePickerEvent, DateRangePickerCommand>; readonly controls: { value: boolean; highlighted: boolean; open: boolean }; readonly granularity: DateRangePickerValueGranularity; readonly referenceYear: number;
  public constructor(options: DateRangePickerOptions, runtime: TerminalTemporalController<DateRangePickerState, DateRangePickerEvent, DateRangePickerCommand>, controls: { value: boolean; highlighted: boolean; open: boolean }, granularity: DateRangePickerValueGranularity) { this.options = options; this.runtime = runtime; this.controls = controls; this.granularity = granularity; this.referenceYear = runtime.getSnapshot().state.calendar.highlighted.year; }
  ...
  public handleEvent(event: DateRangePickerEvent | NonNullable<ReturnType<typeof toPeriodPickerEvent>>): boolean { const result = this.runtime.handle(event as DateRangePickerEvent); if (result.ok) this.options.onUpdate?.(); return result.ok; }
  public handleKeyboardInput(input: TerminalKeyboardInput): boolean { const event: DatePickerEvent | ReturnType<typeof toPeriodPickerEvent> = this.granularity === 'date' ? toDatePickerEvent(input) : toPeriodPickerEvent(input, this.granularity, this.referenceYear); return event !== null && this.handleEvent(event); }
}
