...
function tryCreateSelectConnection<ID extends StableID>(options: SelectOptions<ID>): Result<SelectConnection<ID>> {
  const domain = tryCreateSequence(options.items); if (!domain.ok) return domain; const disabled = createDisabledItems(domain.value, options.disabledItems); if (!disabled.ok) return disabled; const supplied = options.policies?.eligible; const policies: SelectPolicies<ID> = { ...options.policies, eligible: (id) => !disabled.value.has(id) && (supplied?.(id) ?? true) }; const controlled = { value: options.value !== undefined, highlighted: options.highlightedValue !== undefined, open: options.open !== undefined };
  let connection: TerminalSelectConnection<ID> | undefined;
  const runtime = createSemanticController<SelectState<ID>, SelectEvent<ID>, SelectCommand<ID>, SelectEffect<ID>>({ interaction: options, interactionIntent: (event) => event === 'select' || (typeof event === 'object' && event.type === 'select') ? 'mutate' : 'navigate', initial: tryCreateSelectState(domain.value, { value: options.value === undefined ? options.defaultValue ?? null : options.value, ...(options.highlightedValue !== undefined ? { current: options.highlightedValue } : options.defaultHighlightedValue !== undefined ? { current: options.defaultHighlightedValue } : {}), open: options.open ?? options.defaultOpen ?? false }), reducer: (state, event) => applySelectEvent(domain.value, state, event, policies), reconcile: (previous, proposed) => tryCreateSelectState(domain.value, { value: controlled.value ? previous.choice.selection.selected[0] ?? null : proposed.choice.selection.selected[0] ?? null, current: controlled.highlighted ? previous.choice.cursor.current : proposed.choice.cursor.current, open: controlled.open ? previous.open : proposed.open }), notify: [
  ...
  ], toEffect: (command) => command.type === 'focus' ? { type: 'move-highlight', id: command.id } : { type: 'close-popup' }, complete: () => connection?.complete() });
  if (!runtime.ok) return runtime;
  connection = new TerminalSelectConnection(options, domain.value, runtime.value, controlled);
  return { ok: true, value: connection };
}
class TerminalSelectConnection<ID extends StableID> implements SelectConnection<ID> {
  readonly #options: SelectOptions<ID>; readonly #domain: Sequence<ID>; readonly #runtime: SemanticController<SelectState<ID>, SelectEvent<ID>, SelectEffect<ID>>; readonly #controlled: { value: boolean; highlighted: boolean; open: boolean }; #publishedRevision: number;
  constructor(options: SelectOptions<ID>, domain: Sequence<ID>, runtime: SemanticController<SelectState<ID>, SelectEvent<ID>, SelectEffect<ID>>, controlled: { value: boolean; highlighted: boolean; open: boolean }) { this.#options = options; this.#domain = domain; this.#runtime = runtime; this.#controlled = controlled; this.#publishedRevision = runtime.getSnapshot().revision; }
  getSnapshot() { return this.#runtime.getSnapshot(); }
  syncControlledValues(values: { readonly value?: ID | null; readonly highlightedValue?: ID | null; readonly open?: boolean }) { if (this.#controlled.value !== (values.value !== undefined) || this.#controlled.highlighted !== (values.highlightedValue !== undefined) || this.#controlled.open !== (values.open !== undefined)) return { ok: false as const, error: { class: 'construction' as const, code: 'controlled-shape-mismatch' as const, message: 'Controlled select values must preserve their construction-time shape.' } }; const state = this.#runtime.getSnapshot().state; const result = this.#runtime.replace(tryCreateSelectState(this.#domain, { value: this.#controlled.value ? values.value ?? null : state.choice.selection.selected[0] ?? null, current: this.#controlled.highlighted ? values.highlightedValue ?? null : state.choice.cursor.current, open: this.#controlled.open ? values.open ?? false : state.open })); if (result.ok) this.#publishUpdate(); return result; }
  handleEvent(event: SelectEvent<ID>) { return this.#runtime.handle(event).ok; }
  handleKeyboardInput(input: TerminalKeyboardInput) { const event = toSelectEvent<ID>(input); return event === null ? false : this.handleEvent(event); }
  complete(): void { this.#publishUpdate(); }
...
function toSelectEvent<ID extends StableID>(input: TerminalKeyboardInput): SelectEvent<ID> | null { if (input.key === 'down') return 'next'; if (input.key === 'up') return 'previous'; if (input.key === 'home') return 'first'; if (input.key === 'end') return 'last'; if (input.key === 'enter' || input.key === 'space') return 'select'; if (input.key === 'escape') return 'close'; return null; }
