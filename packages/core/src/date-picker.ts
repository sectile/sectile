...
export function createDatePickerState(input: DatePickerStateInput = {}): Result<DatePickerState> {
  ...
  return ok(pickerState(value, validHighlight.value, Object.freeze({ year: view.year, month: view.month }), input.open ?? false));
}
...
export function applyDatePickerEvent(state: DatePickerState, event: DatePickerEvent, policies: DatePickerPolicies = {}): Result<DatePickerUpdate> {
  ...
  if (event === 'open' || event === 'close' || event === 'toggle') {
    ...
    return createMachineUpdate(pickerState(state.value, state.highlighted, state.view, open), [{ type: 'open-changed', open }]);
  }
  ...
  return createMachineUpdate(pickerState(state.value, eligible.value, view, state.open), [{ type: 'highlight-changed', value: eligible.value }]);
}
...
function commit(value: DateValue | null, state: DatePickerState, policies: DatePickerPolicies, close: boolean): Result<DatePickerUpdate> {
  if (value === null) {
    ...
    return createMachineUpdate(pickerState(null, state.highlighted, state.view, open), [
  ...
  return createMachineUpdate(pickerState(valid.value, valid.value, view, open), [
...
function pickerState(value: DateValue | null, highlighted: DateValue, view: DatePickerView, open: boolean): DatePickerState { return Object.freeze({ value, highlighted, view, open }); }
