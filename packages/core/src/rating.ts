...
import { applyRadioGroupEvent, createRadioGroupState, type RadioGroupCommand, type RadioGroupPolicies, type RadioGroupState, type RadioGroupUpdate,tryCreateRadioGroupState } from './radio-group.js';
...
export function createRatingState<ID extends StableID>(domain: Sequence<ID>, value: ID | null = null): RatingState<ID> {
  return unwrap(tryCreateRatingState(domain, value));
}

export function tryCreateRatingState<ID extends StableID>(domain: Sequence<ID>, value: ID | null = null): Result<RatingState<ID>> {
  return tryCreateRadioGroupState(domain, { current: value, selected: value === null ? [] : [value] });
}
