...
export function createCheckedState(value: CheckedValue = false, policies: CheckedPolicies = {}): CheckedState {
  return unwrap(tryCreateCheckedState(value, policies));
}

export function tryCreateCheckedState(value: CheckedValue = false, policies: CheckedPolicies = {}): Result<CheckedState> {
...
export function applyCheckedEvent(state: CheckedState, event: CheckedEvent, policies: CheckedPolicies = {}): Result<MachineUpdate<CheckedState, CheckedCommand>> {
  const validState = tryCreateCheckedState(state.checked, policies);
  ...
  const created = tryCreateCheckedState(next, policies); if (!created.ok) return created;
