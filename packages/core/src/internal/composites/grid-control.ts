...
export function createGridState<ID extends StableID>(grid: Grid<ID>, input: GridStateInput<ID> = {}): Result<GridState<ID>> {
  ...
  return ok(gridState(current, selection.value, editMode));
}
...
export function applyGridEvent<ID extends StableID>(grid: Grid<ID>, state: GridState<ID>, event: GridEvent<ID>, policies: GridPolicies<ID> = {}): Result<GridUpdate<ID>> {
  ...
  if (typeof event === 'object') {
    ...
    if (event.type === 'focus') return createMachineUpdate(gridState(event.id, state.selection, 'navigation'), [{ type: 'focus', id: event.id }]);
    const selection = selectOne(state.selection, event.id, domain);
    if (event.type === 'select') return createMachineUpdate(gridState(event.id, selection, 'navigation'), [{ type: 'focus', id: event.id }]);
    return createMachineUpdate(gridState(event.id, selection, 'editing'), [{ type: 'focus', id: event.id }, { type: 'begin-edit', id: event.id }]);
  }
  ...
  if (event === 'commit-edit' || event === 'cancel-edit') {
    ...
    return createMachineUpdate(gridState(current, state.selection, 'navigation'), [{ type: event, id: current }]);
  }
  ...
  if (event === 'start-edit') {
    ...
    return createMachineUpdate(gridState(current, state.selection, 'editing'), [{ type: 'begin-edit', id: current }]);
  }
  if (event === 'select') {
    if (current === null) return fail('transition-rejection', 'no-cursor', 'Grid selection requires a cursor.');
    return createMachineUpdate(gridState(current, selectOne(state.selection, current, domain), 'navigation'));
  }
  ...
  return target === null ? createMachineUpdate(state) : createMachineUpdate(gridState(target, state.selection, 'navigation'), [{ type: 'focus', id: target }]);
}
...
function gridState<ID extends StableID>(current: ID | null, selection: SelectionState<ID>, editMode: GridEditMode): GridState<ID> {
  return Object.freeze({ cursor: createCursorState(current), selection, editMode });
}
