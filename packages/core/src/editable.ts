...
export function createEditableState(value: string = '', draft: string = value, editing: boolean = false): Result<EditableState> {
...
export function applyEditableEvent(state: EditableState, event: EditableEvent, policies: EditablePolicies = {}): Result<EditableUpdate> {
  const valid = createEditableState(state.value, state.draft, state.editing);
