...
The policy checker validates build configuration, built files, resolving `sourceMappingURL` references, and `npm pack --dry-run` contents for all nine packages. It also rejects unmapped JavaScript, dangling or inline maps, declaration maps, stale declaration references, and embedded source content.
...
The declaration-map removal eliminated 404 files and 913,211 bytes. JavaScript maps remain complete for all 441 published JavaScript files.
