...
`v0.14.1` is the compatibility bridge from synchronized versions to independent package versions. It must be published with `pnpm release patch` before any independent release. This final synchronized release changes published internal dependency and peer ranges to caret ranges. The release script rejects further synchronized releases after this tag.
