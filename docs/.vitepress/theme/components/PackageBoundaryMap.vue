...
const copy = computed(() => isKorean.value ? {
  ...
  form: '폼 조정',
  formRole: '필드, 접근 가능한 오류, 검증, 제출과 초기화를 연결합니다.',
  formExamples: ['필드', '오류', '제출', '초기화'],
  temporal: '날짜와 시각',
  ...
  chart: '차트 투영',
  chartRole: '차트 데이터에서 제한된 geometry와 결정적인 상호작용 상태를 계산합니다.',
  chartExamples: ['점', '선', '막대', '히트맵', '파이'],
  ...
  form: 'Form coordination',
  formRole: 'Connects fields, accessible errors, validation, submission, and reset.',
  formExamples: ['fields', 'errors', 'submission', 'reset'],
  ...
  chartRole: 'Computes bounded geometry and deterministic interaction state from chart data.',
  chartExamples: ['points', 'lines', 'bars', 'heatmaps', 'pie'],
...
const domains = computed(() => [
  ...
  {
    id: 'form',
    packageName: '@sectile/form',
    name: copy.value.form,
    role: copy.value.formRole,
    examples: copy.value.formExamples,
    icon: ListChecks,
  },
  {
    id: 'temporal',
  ...
  {
    id: 'chart',
    packageName: '@sectile/chart',
    name: copy.value.chart,
    role: copy.value.chartRole,
    examples: copy.value.chartExamples,
    icon: PieChart,
  },
].map((domain) => ({
  ...domain,
