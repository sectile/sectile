...
function notify(push: (input: { id: string; title: string; description: string; kind: 'success' | 'warning' | 'error'; durationMs?: number | null }) => void, kind: 'success' | 'warning' | 'error'): void { sequence.value += 1; const id = `${kind}-${sequence.value}`; push({ id, title: kind === 'success' ? 'Release saved' : kind === 'warning' ? 'Review required' : 'Deployment failed', description: `Notification ${sequence.value}`, kind, ...(props.persistent ? { durationMs: null } : {}) }); revision.value += 1; entries.value = [{ revision: revision.value, event: 'push', accepted: true, effects: [`announce-toast id=${id}`] }, ...entries.value].slice(0, 12); }
</script>
<template>
      ...
      <div class="toast-example">
        ...
        <ToastViewport class="toast-viewport">
          <ToastRoot v-for="item in toasts" :key="item.id" :value="item.id" class="toast-item">
            <div><ToastTitle /><ToastDescription /></div><ToastClose class="icon-control secondary">×</ToastClose>
          </ToastRoot>
        </ToastViewport>
      </div>
    </ToastProvider>
