...
const partPurposes = Object.freeze<Record<string, Readonly<{ en: string; ko: string }>>>({
  ...
  reset: copy('Resets native form values and coordinated form state.', '네이티브 폼 값과 연결된 폼 상태를 초기화합니다.'),
  submit: copy('Submits the native form through coordinated validation.', '연결된 검증을 거쳐 네이티브 폼을 제출합니다.'),
...
export const componentAnatomy = Object.freeze<Record<string, ComponentAnatomyDefinition>>({
  ...
  stepper: anatomy('stepper', ['root', 'list', 'step', 'indicator', 'content', 'previous', 'next'], {
    previous: {
      attributes: [['data-target-value', '<value>'], ['data-disabled', '']],
      purpose: copy('Activates and focuses the previous enabled step.', '이전의 활성화 가능한 단계를 선택하고 포커스를 옮깁니다.'),
    },
    next: {
      attributes: [['data-target-value', '<value>'], ['data-disabled', '']],
      purpose: copy('Activates and focuses the next enabled step.', '다음의 활성화 가능한 단계를 선택하고 포커스를 옮깁니다.'),
    },
  }),
