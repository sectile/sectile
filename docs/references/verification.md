...
The primitives implementation verifier uses seed `0x5EC71E`, compares 2,000 generated models per structure and internal cursor, selection, expansion, text, listbox, slider, calendar, tree-view, and combobox theories, covers invalid construction and controlled-snapshot vectors, and checks normal observations, query absence, projections, movement, scan rejection, exact range arithmetic, tree expansion, state reconciliation, selection operations, expansion transitions, UTF-16 replacement, composition transitions, listbox command traces, bounded slider transitions, calendar grid navigation, external page commands, visible tree navigation, committed-query combobox filtering, composition-safe candidate acceptance, and 124 revision-wrapper cases. The treegrid reference suite separately checks 5,881 bounded row-tree/cell-grid models and 276,720 transitions. The primitives gate runs its deterministic implementation verifier twice, requires byte-identical output, and compares it with `packages/primitives/verification/implementation-verification.json`. The root cross-host suite independently compares 160,000 listbox, slider, calendar, tree-view, treegrid, combobox, and text-editing operations through `@sectile/dom` and `@sectile/terminal` package exports.
