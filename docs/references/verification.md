...
The implementation verifier uses seed `0x5EC71E`, compares 2,000 generated models per structure and internal cursor, selection, and expansion state theories, covers invalid construction and controlled-snapshot vectors, and checks normal observations, query absence, projections, movement, scan rejection, exact range arithmetic, tree expansion, cursor and selection reconciliation, selection operations, and expansion transitions. The repository gate runs it twice, requires byte-identical output, and compares it with `verification/implementation-verification.json`.
