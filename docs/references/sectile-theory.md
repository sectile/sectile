...
`DateValue`, `TimeValue`, `DateTimeValue`는 instant가 아닌 달력·벽시계 값이다. `DateValue`는 ISO 8601 proleptic Gregorian의 `year/month/day`, `TimeValue`는 24시간제 하루 안의 `hour/minute/second/millisecond`만 소유한다. `DateTimeValue`는 둘의 product이며 time zone, UTC offset, locale, daylight-saving transition을 저장하지 않는다. 이 분리는 Temporal의 `PlainDate`·`PlainTime`·`PlainDateTime`과 같은 의미 경계다.[R27]
...
날짜 비교와 증감은 host `Date` 객체 없이 정수 Gregorian 연산으로 결정한다. 월·연도 증감에서 대상 월에 같은 day가 없으면 그 달의 마지막 날로 clamp한다. day 증감은 달과 연도 경계를 넘는다. 독립 `TimeValue` 증감은 24시간 안에서 wrap하지만, `DateTimeValue`의 시간 segment 증감은 자정 경계를 날짜로 carry한다. 달력 날짜를 instant로 바꾸거나 time zone을 적용하는 일은 정책 또는 상위 application 책임이다.

DateField, TimeField, DateTimeField는 committed value와 `TextEditingState`를 분리한다. 입력 중 draft, selection, caret, IME composition은 Text theory를 그대로 사용한다. commit만 strict ISO text를 값으로 바꾸며 실패 시 기존 값과 command를 보존한다. DateTimeField의 canonical text는 `YYYY-MM-DDTHH:mm`이고 필요한 경우 seconds와 milliseconds를 보존한다. 위·아래 입력은 완전하게 parse 가능한 draft에서만 caret이 속한 segment를 증감한다. 일반 text editing key는 가로채지 않는다.
