...
도메인이 바뀌면 Sectile은 제어 중인 상태를 새 도메인에 먼저 맞춥니다. 선택·커서·활성 데이터의 ID가 사라졌거나 제어 중인 `view`를 update 규칙에 따라 새 축 도메인으로 옮겨야 하면, 컨트롤러는 새 도메인에서 유효한 값을 반영하고 해당 변경 요청을 보냅니다. 앱은 이전 도메인의 값을 계속 전달하지 말고 요청받은 값을 다시 동기화해야 합니다. 제어 중인 `view`의 정착 알림은 앱이 동기화한 값을 컨트롤러가 받아들인 뒤에만 발생하므로 DOM의 범위 안내도 제안값이 아니라 확정된 범위를 읽습니다.
