---
title: 제출과 값 기준 관리
description: Vue에서 네이티브·관리형 제출을 선택하고 FormData와 스키마를 처리하며 저장된 값을 새 기준으로 삼습니다.
---
...
`defineFormSubmission()`은 선택적 스키마와 제출 함수를 함께 묶습니다. 반환값을 `v-bind`로 `FormRoot`에 전달합니다.
...
스키마가 없으면 `formData`를 우선 사용하세요. 파일, 반복 이름, checkbox·radio 생략, 비활성화된 컨트롤, 클릭한 제출 버튼을 정확히 표현합니다.
...
Standard Schema를 전달하면 `values`는 스키마가 변환한 출력 타입입니다.

```ts
const submission = defineFormSubmission({
  schema: checkoutSchema,
  onSubmit: ({ values }) => {
    values.quantity // 스키마 변환 뒤 number로 추론
...
| --- | --- |
| `touched` | 필드를 조작한 기록 |
| `validation` | 브라우저·검증 함수·스키마의 검증 상태와 오류 |
