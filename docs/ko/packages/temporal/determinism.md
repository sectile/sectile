...
DOM과 Terminal 연결 함수는 기준 날짜를 생략했을 때 실행 환경의 오늘 날짜를 넣을 수 있습니다. Vue 선택기 루트에는 기준 날짜를 직접 전달하고, `TemporalProvider`로 하위 영역에 하나의 기준 날짜를 공유할 수 있습니다. 브라우저에서만 실행되는 화면은 편의 기본값을 써도 되지만 테스트, 저장된 화면, 작업자 스레드, 서버 렌더링에는 고정된 값을 전달해야 합니다.
