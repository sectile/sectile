...
const profiles = Object.freeze({
  ...
  datePicker: [
    ...
    row('Escape', 'Close the calendar without selecting another date.', '현재 날짜를 유지한 채 달력을 닫습니다.'),
  ],
  monthPicker: [
    ...
    row('Escape', 'Close the month grid without changing the value.', '현재 값을 유지한 채 월 격자를 닫습니다.'),
  ],
  yearPicker: [
    ...
    row('Escape', 'Close the year grid without changing the value.', '현재 값을 유지한 채 연도 격자를 닫습니다.'),
  ],
  popup: [
  ...
  combobox: [
    ...
    row('Escape', 'Close the popup without replacing the accepted value.', '확정된 값을 유지한 채 팝업을 닫습니다.'),
    row('Text input', 'Filter options without breaking IME composition.', 'IME 조합 입력을 유지하며 항목을 검색합니다.'),
  ],
  grid: [
  ...
  readonly: [
    row('None', 'The range display is read-only and defines no keyboard interaction.', '범위 표시는 읽기 전용 의미와 값만 제공합니다.'),
  ],
  toast: [
...
const profileByComponent = Object.freeze({
  ...
  'menu-button': 'menu', menubar: 'menubar', meter: 'readonly', 'meter-group': 'readonly', progress: 'readonly', 'multi-thumb-slider': 'range', 'navigation-menu': 'menubar',
...
const accessibilityByComponent = Object.freeze({
  ...
  'color-picker': ['Text inputs and sliders expose labels, ranges, current values, and channel purpose without relying on color alone.', '텍스트 입력과 슬라이더가 이름, 범위, 현재 값, 색상 채널 역할을 함께 노출합니다.'],
  ...
  'date-range-field': ['Separate labeled start and end inputs expose a single ordered range without hiding endpoint errors.', '시작과 종료 입력에 각각 이름을 제공하고 양 끝의 오류와 순서 있는 범위를 함께 노출합니다.'],
  ...
  'range-calendar': ['The always-visible grid exposes both range endpoints and every date between them without a popup trigger.', '항상 보이는 격자가 범위의 양 끝과 그 사이 날짜를 화면에 바로 노출합니다.'],
  ...
  editable: ['Preview and input states remain distinguishable; invalid drafts are announced on the real input.', '미리보기와 입력 상태를 구분하고 검증 오류는 실제 입력 요소에서 전달합니다.'],
  ...
  listbox: ['The labeled listbox exposes active, selected, and disabled option state without moving DOM focus to every item.', '이름이 있는 목록 상자가 DOM 포커스를 유지하며 현재·선택·비활성 상태를 노출합니다.'],
  ...
  meter: ['The named read-only meter exposes its minimum, maximum, current value, and formatted value text without inventing ARIA threshold attributes.', '이름이 있는 읽기 전용 meter가 표준 ARIA 속성으로 최솟값·최댓값·현재 값·형식화된 값을 노출합니다.'],
  'meter-group': ['One named group contains ordered, individually named read-only meters; visual track and legend parts add no duplicate aggregate announcement.', '이름이 있는 하나의 그룹 안에 순서와 이름이 있는 읽기 전용 meter를 두며 시각적 트랙과 범례는 같은 집계 의미를 공유합니다.'],
  progress: ['The named progress bar exposes zero and its maximum, omitting the current value and value text while progress is indeterminate.', '이름이 있는 진행 표시줄은 0과 최댓값을 노출하고 진행량 미정 상태에서는 현재 값과 값 텍스트를 생략합니다.'],
  ...
  pagination: ['Page links or buttons keep native activation semantics and identify the current page without relying on position alone.', '페이지 링크나 버튼이 기본 실행 의미와 현재 페이지 속성을 함께 제공합니다.'],
  ...
  tooltip: ['The tooltip is associated with its trigger as a description and never receives focus itself.', '도움말을 실행 요소의 설명으로 연결하고 포커스는 실행 요소에 유지합니다.'],
