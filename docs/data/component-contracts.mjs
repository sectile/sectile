...
const profiles = Object.freeze({
  ...
  readonly: [
    row('None', 'The range display is read-only and defines no keyboard interaction.', '범위 표시는 읽기 전용이며 키보드 조작을 정의하지 않습니다.'),
  ],
  toast: [
...
const accessibilityByComponent = Object.freeze({
  ...
  'meter-group': ['One named group contains ordered, individually named read-only meters; visual track and legend parts add no duplicate aggregate announcement.', '이름이 있는 하나의 그룹 안에 순서와 이름이 있는 읽기 전용 meter를 두며 시각적 트랙과 범례는 집계 값을 중복 안내하지 않습니다.'],
  progress: ['The named progress bar exposes zero and its maximum, omitting the current value and value text while progress is indeterminate.', '이름이 있는 진행 표시줄은 0과 최댓값을 노출하고 진행량을 알 수 없을 때 현재 값과 값 텍스트를 생략합니다.'],
