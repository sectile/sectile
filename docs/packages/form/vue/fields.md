...
description: Use native inputs, Sectile Vue components, groups, nested names, and external controls in one form.
---
...
Use the same pattern for a checkbox group. Put `required` on an individual native checkbox only when that specific choice must be selected.

A fieldset still represents one answer and therefore one `FormField`. After a failed submission, Form focuses the first enabled control in the group that fails a native constraint. If only a schema or application rule makes the group invalid, Form focuses its first enabled control instead.

Wrap each child in a separate `FormField` when it represents an independent answer with its own error message. For an application-specific control made from several elements, set its `focusTarget` with [`useCompositeFormControl()`](./custom-controls#composite-controls).
...
Use `FormField` when you also need connected labels, descriptions, errors, form state, or first-invalid focus. A named control without `FormField` is included in `FormData`, but Form does not track it as a field, so it does not contribute to `dirty` or `touched`.
