...
description: Choose native or managed submission in Vue, handle FormData and schemas, and establish a new value baseline after saving.
---
...
Configured form and field issues remain in either case. `dirty` always becomes `false` because the current field and control values become the new baseline.
