...
For a controlled request, Vue publishes the latest committed snapshot first, then runs the generic command callback, then publishes the owner proposal. `ChartRoot` publishes that proposal through the matching `update:*` event. `useChart()` writes the controlled ref in the same post-command phase and then runs its state-specific `on*Change` callback. Command callbacks therefore observe the previously accepted controlled value, while state-specific change callbacks observe the proposed owner value. Controlled ownership is fixed for the component lifetime; remount to change a prop between controlled and uncontrolled ownership. A callback error does not skip the remaining matching publication phases, and imperative composables rethrow the first synchronous callback error after publication completes.
